...
- **Touch on chest breaks sidebar navigation:** After touching the screen while a chest is open, controller snap navigation can no longer reach the chest sort button or close button. The game likely resets internal snap state or component wiring on touch. **Root cause unknown — needs investigation.** May need to re-run `FixSnapNavigation()` after touch events, or save/restore the `currentlySnappedComponent` like the inventory touch guard.
