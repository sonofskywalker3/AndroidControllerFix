...
namespace AndroidConsolizer.Patches
{
    ...
    internal static class InventoryManagementPatches
    {
        ...
        /// <summary>
        /// Handle Y button press for single-stack pickup.
        /// Returns true if the input was handled and should be suppressed.
        /// </summary>
        public static bool HandleYButtonPressed(GameMenu gameMenu, IMonitor monitor)
        {
            Monitor = monitor;
            ...
            try
            {
                ...
                // Only handle inventory slots (0-35)
                if (slotId < 0 || slotId >= Game1.player.Items.Count)
                    return false;
                ...
                // Start Y button hold tracking
                IsYButtonHeld = true;
                YButtonHoldTicks = 0;

                // Pick up single item
                return PickupSingleItem(inventoryPage, slotId, item);
            }
            catch (Exception ex)
            {
                Monitor.Log($"InventoryManagement HandleYButtonPressed error: {ex.Message}", LogLevel.Error);
                return false;
            }
        }
        ...
        /// </summary>
        public static void HandleYButtonReleased()
        {
            IsYButtonHeld = false;
            YButtonHoldTicks = 0;
        }
        ...
        /// <summary>
        /// Try to pick up a single item from the currently snapped slot (for hold-repeat).
        /// </summary>
        private static void TryPickupSingleFromCurrentSlot(GameMenu gameMenu)
        {
            try
            {
                var inventoryPage = gameMenu.pages[GameMenu.inventoryTab] as InventoryPage;
            ...
            {
                if (ModEntry.Config.VerboseLogging)
                    Monitor?.Log($"InventoryManagement: TryPickupSingle error: {ex.Message}", LogLevel.Debug);
            }
        }
